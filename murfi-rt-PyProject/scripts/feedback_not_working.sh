...
then
    ...
    singularity exec murfi2-sif_XA30.sif murfi -f $subj_dir/xml/2vol.xml
...
clear
    ...
    singularity exec murfi2-sif_XA30.sif murfi -f $subj_dir_absolute/xml/rtdmn.xml
...
clear
    ...
    singularity exec murfi2-sif_XA30.sif murfi -f $subj_dir/xml/rest.xml
...
clear
	...
	# Look if Tw1 image is awailable for registration 
	Tw1_image=$(find ${subj_dir_absolute}/rest/ -type f \( -iname "*T1w*" \) | head -1)
	if [ ${run_volumes} -ne 0 ]
        then
            runstring="${runstring}\nRun ${i}: ${run_volumes} volumes"
	
	if [ -z "$Tw1_image" ]; then
		runstring="${runstring}\nTw1 image: not available"
	else
		Tw1_image_name=$(basename $(find ${subj_dir_absolute}/rest/ -type f \( -iname "*T1w*" \) | head -1))
		runstring="${runstring}\nTw1 image: ${Tw1_image_name}"
	fi
	...
	#reference_vol_for_ica=$template_lps_path
        ...
        template_lps_path=$template_lps_path
        #reference_vol_for_ica=$subj_dir_absolute/rest/$subj'_'$ses'_task-rest_run-01_bold_mcflirt_median_bet.nii.gz'
