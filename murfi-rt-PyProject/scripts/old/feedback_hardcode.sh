...
then
    ...
    then
        ...
        then
            ...
            flirt -in $infile -ref $examplefunc -out $ica_directory/melodic_IC_examplefunc.nii -init $mni_lps2example_func_mat -applyxfm -interp trilinear
    ...
    # Only do registration for multi-run (single-run is already in native space)
    ...
    then
        ...
        # For single-run, need to resample template networks to native space
        mkdir -p ${ica_directory}/reg
        
        # Calculate registration from MNI to native space
        mni2native_mat=${ica_directory}/reg/mni2native.mat
        mni_lps2xample_func=${ica_directory}/reg/mni_lps2native.nii
        flirt -in MNI152_T1_2mm_LPS_brain -ref ${examplefunc} -out ${mni_lps2xample_func} -omat ${mni2native_mat}
    ...
    percentile_cen=$(python -c "print(100*(1-1000/${voxels_in_cen}))")
...
then
    ...
    #epi_reg --epi=$examplefunc --t1=${latest_ref}_brain --t1brain=${latest_ref}_brain --out=$subj_dir/xfm/epi2reg/rest2studyref
    #cp ../subjects/sub-rtbpd015/xfm/epi2reg/rest2studyref.nii ../subjects/sub-rtbpd015/xfm/epi2reg/rest2studyref_brain.nii
    ...
    do 
        ...
        # Check orientations before registration
        echo "Checking orientations..."
        fslhd $subj_dir/mask/${mask_name}_native_rest.nii | grep qform
        fslhd ${latest_ref} | grep qform
    
        # Force reorientation to standard if needed
        fslreorient2std $subj_dir/mask/${mask_name}_native_rest.nii $subj_dir/mask/${mask_name}_native_rest_reorient.nii
        fslreorient2std ${latest_ref} ${latest_ref}_reorient
    
        # Use reoriented files for registration
        flirt -in $subj_dir/mask/${mask_name}_native_rest_reorient.nii \
              -ref ${latest_ref}_reorient \
              -out $subj_dir/mask/${mask_name} \
              -init $subj_dir/xfm/epi2reg/rest2studyref.mat \
              -applyxfm -interp nearestneighbour -datatype short    
    
        # erode 2vol brain mask one voxel
        fslmaths ${latest_ref}_brain_mask -ero ${latest_ref}_brain_mask_ero1

        # binarize masks based on eroded 2vol brain mask
        fslmaths $subj_dir/mask/${mask_name}.nii.gz -mul ${latest_ref}_brain_mask_ero1 $subj_dir/mask/${mask_name}_final.nii.gz -odt short
    
        # Check if output is compressed or not, and handle accordingly
        ...
        elif [ -f "$subj_dir/mask/${mask_name}.nii.gz" ]; then
            gunzip -f $subj_dir/mask/${mask_name}.nii.gz
        fi
        # If already .nii, no need to gunzip
    
        # Clean up temporary files
        rm -f $subj_dir/mask/${mask_name}_native_rest_reorient.nii*
        rm -f ${latest_ref}_reorient.nii*
    done
