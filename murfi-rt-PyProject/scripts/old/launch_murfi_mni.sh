...
check_required_files() {
    ...
    if [[ -f "$xml_dir/2vol.xml" ]] && [[ -f "$xml_dir/rest.xml" ]] && [[ -f "$xml_dir/rtdmn.xml" ]] && [[ -f "$xml_dir/neurofeedback_Bay1_XA30.xml" ]]; then
        xml_ready=true
    fi
    
    # Check masks status
    if [[ -d "$mask_dir" ]]; then
        mask_count=$(find "$mask_dir" -name "*.nii.gz" -type f 2>/dev/null | wc -l)
...
main_gui() {
    ...
    if [[ -z $MURFI_SUBJECT_NAME ]] || [[ "$1" == "--new-subject" ]]; then
        ...
        if [[ -d "$subject_dir" ]]; then
            ...
            if [[ -d "../subjects/rtbpd_template" ]]; then
                cp -r ../subjects/rtbpd_template/* "$subject_dir/"
                log_message "New directory created with template files for $participant_id"
...
execute_rtbpd_step() {
    ...
    case "$step" in
        ...
        'transferpre')
            ...
            echo "[$(date +%F_%T)] source feedback_mni.sh $participant_id feedback transferpre" >> "$cmd_log"
            clear
            source feedback_mni.sh "$participant_id" feedback
            ;;
        'transferpost')
            log_message "Running transfer post-training scan for $participant_id"
            echo "[$(date +%F_%T)] source feedback_mni.sh $participant_id feedback transferpost" >> "$cmd_log"
            clear
            source feedback_mni.sh "$participant_id" feedback
            ;;
        *)
            # Run standard feedback_mni.sh steps
            log_message "Running feedback_mni.sh for $participant_id with step $step"
            echo "[$(date +%F_%T)] feedback_mni.sh $participant_id $step" >> "$cmd_log"
            clear
            source feedback_mni.sh "$participant_id" "$step"
            ;;
    esac
